...
abstract class SessionScenario {
    ...
    @Test void indexedSessionKeyeventsStayIsolated() throws Exception {
        ...
        assertTrue(awaitKeyevent("expired", store.channel("sessions:expires:" + ttlSession)), "Missing expired keyevent");

        List<Integer> peers = barrier(round, "ready", worker);
        Path exchange = sessionCoordPath(round, "exchange");
    ...
    boolean awaitKeyevent(String kind, String keyBody) throws InterruptedException {
        String channel = store.redisKeyeventChannel(kind);
        long deadline = System.nanoTime() + TimeUnit.SECONDS.toNanos(5);
        while (System.nanoTime() < deadline) {
            String event = indexer.keyEvents.poll(50, TimeUnit.MILLISECONDS);
            if (event == null) continue;
            if (event.startsWith(channel + " ") && event.contains(keyBody)) return true;
            indexer.keyEvents.add(event);
        }
        return false;
    }

    static Path sessionCoordPath(int round, String phase) {
    ...
    List<Integer> barrier(int round, String phase, int worker) throws Exception {
        ...
        Path directory = sessionCoordPath(round, phase);
