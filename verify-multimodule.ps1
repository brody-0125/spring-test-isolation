...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Multimodule tests failed; inspect $log" }
    ...
    foreach ($module in @('verification','verification-peer')) {
        ...
        if ($total -ne 11) { throw "Expected eleven tests for $module; got $total" }
    }
    Write-Output 'PASS: both modules share one container pair; 22 tests passed.'
