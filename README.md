...
Unsupported: `@Nested`, `@ContextHierarchy`, Redis Cluster/Sentinel — see [VERIFICATION.md](VERIFICATION.md). TestNG is supported on native `useTestNG()` tasks, and Kotest on JUnit Platform (`kotest-runner-junit5`), when classes stay sequential inside each worker.
