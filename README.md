...
Unsupported in 1.0.0: `@Nested` with its own context, `@ContextHierarchy`, Redis Cluster/Sentinel, TestNG/Kotest — see [VERIFICATION.md](VERIFICATION.md). `@Nested` that inherits the enclosing class context is supported; nested classes share worker storage until the enclosing class ends.
